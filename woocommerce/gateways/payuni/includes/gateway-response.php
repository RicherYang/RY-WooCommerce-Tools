...
final class RY_WT_WC_PAYUNi_Gateway_Response extends RY_WT_PAYUNi_Api
{
    ...
    protected function ipn_request_is_valid(array $ipn_info): bool
    {
        ...
        if ($check_value) {
            ...
            RY_WT_WC_PAYUNi_Gateway::instance()->log('IPN request failed', WC_Log_Levels::NOTICE, ['get' => wp_unslash($_GET), 'post' => wp_unslash($_POST)]);
        }
    ...
    public function doing_callback($ipn_info)
    {
        ...
        if ($order = wc_get_order($order_ID)) {
            if ($order->get_meta('_payuni_MerTradeNo', true) === $info_value['MerTradeNo']) {
                ...
                RY_WT_WC_PAYUNi_Gateway::instance()->log('Order TradeNo mismatch for #' . $order->get_id(), WC_Log_Levels::WARNING);
