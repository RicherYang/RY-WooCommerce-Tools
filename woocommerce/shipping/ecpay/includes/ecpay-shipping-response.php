...
class RY_ECPay_Shipping_Response extends RY_ECPay_Shipping_Api {
	public static function init() {
		...
		add_action('valid-shipping-request', array(__CLASS__, 'shipping_callback'));
	}
	...
	public static function shipping_callback($ipn_info) {
		$order_id = self::get_order_id($ipn_info, get_option(RY_WT::$option_prefix . 'ecpay_shipping_order_prefix'));
		if( $order = wc_get_order($order_id) ) {
			...
			if( !isset($cvs_info_list[$ipn_info['AllPayLogisticsID']]) ) {
				$cvs_info_list[$ipn_info['AllPayLogisticsID']] = array();
			}
			...
			if( 'yes' === get_option(RY_WT::$option_prefix . 'ecpay_shipping_log_status_change', 'no') ) {
				if( isset($old_info['status']) ) {
					if( $old_info['status'] != $cvs_info_list[$ipn_info['AllPayLogisticsID']]['status'] ) {
						$order->add_order_note(sprintf(__('%s shipping status from %s to %s', RY_WT::$textdomain), $ipn_info['AllPayLogisticsID'], $old_info['status_msg'], $cvs_info_list[$ipn_info['AllPayLogisticsID']]['status_msg']));
					}
				}
			}

			if( in_array($cvs_info_list[$ipn_info['AllPayLogisticsID']]['status'], array(2063, 2073, 3018)) ) {
				$order->update_status('ry-at-cvs');
			}

			if( in_array($cvs_info_list[$ipn_info['AllPayLogisticsID']]['status'], array(2067, 3022)) ) {
				if( 'yes' == get_option(RY_WT::$option_prefix . 'ecpay_shipping_auto_completed', 'yes') ) {
