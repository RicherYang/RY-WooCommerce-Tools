...
final class RY_WT_WC_SmilePay_Shipping_Api extends RY_WT_SmilePay_Api
{
    ...
    public function csv_checkout_form($order, $collection = false)
    {
    ...
    public function get_info_no($order_ID, $get_smse_ID)
    {
        ...
        if (!isset($shipping_list[$get_smse_ID])) {
            return false;
        }
        ...
        $shipping_list = $order->get_meta('_smilepay_shipping_info', true);
        $shipping_list[$get_smse_ID]['PaymentNo'] = (string) $result->paymentno;
        $shipping_list[$get_smse_ID]['ValidationNo'] = (string) $result->validationno;
        $shipping_list[$get_smse_ID]['BookingNote'] = (string) $result->TrackNum;
        $shipping_list[$get_smse_ID]['edit'] = (string) new WC_DateTime();
        $shipping_list[$get_smse_ID]['amount'] = (int) $result->Amount;
        $order->update_meta_data('_smilepay_shipping_info', $shipping_list);
        $order->save();
    }
